...
@section('display')
    menu-item-open
@endsection
@section('room_display')
    menu-item-active
@endsection
@section('title')
    {{ __('lang.room_display') }}
@endsection
@section('toolbar-right')
    <span class="text-muted font-weight-bold mr-4">{{ __('lang.room_display') }}</span>
@endsection
        ...
        <div class=" col-lg-12">
            <div class="card card-custom gutter-b">
                <div class="card-header">
                    <h3 class="card-title">{{ __('lang.room_display') }}</h3>
                </div>
                <div class="card-body p-2">

                    <form action="{{ route('master.displayshow.roomfilter') }}" method="post">
                        @csrf
                        @php
                            $name = $_GET['name'] ?? '';
                            $status = $_GET['status'] ?? '';
                            ...
                            <tbody>
                                @foreach ($rooms as $key => $data)
                                    <tr>
                                        ...
                                        <td class="text-center p-0 px-1 border border-primary">
                                            <a href="{{ route('master.displayshow.roomview',$data->id) }}" class="btn btn-success btn-sm">
                                                <i class="fas fa-tv"></i>
                                            </a>
                                        </td>
                                    </tr>
