...
@endsection
@section('title')
    {{ __('lang.edit_display') }}
@endsection
@section('toolbar-right')
    <span class="text-muted font-weight-bold mr-4">{{ __('lang.edit_display') }}</span>
@endsection
        ...
        <div class="col-lg-4">
            <div class="card card-custom gutter-b">
                <div class="card-header px-2">
                    <h3 class="card-title">{{ __('lang.edit_display') }}</h3>
                </div>
                <form class="form" action="{{ route('master.display.update',$display->id) }}" method="POST"
                    ...
                    <div class="card-body py-2 px-3">
                        <div class="form-group row mb-0">
                            ...
                            <div class="col-lg-12 mt-3 ">
                                ...
                                <select class="form-control text-capitalize multipleOption" name="service[]" multiple
                                    required>
                                    @foreach ($services as $data)
                                        <option value="{{ $data->id }}" {{ collect($display->service)->contains('service_id', $data->id) == $data->id? 'selected' : '' }}>{{ $data->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-lg-12 ">
                                ...
                                <input type="text" name="name"
                                    ...
                                    placeholder="{{ __('lang.name') }}" value="{{$display->name }}" required />
                                @error('name')
                            ...
                            <div class="col-lg-12 ">
                                ...
                                <input type="text" name="deacription"
                                    ...
                                    placeholder="{{ __('lang.deacription') }}" value="{{ $display->deacription }}" />
                                @error('deacription')
                                    <span class="invalid-feedback" role="alert">
                                        <strong class="text-danger">{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="col-lg-12 mt-3 ">
                                ...
                                <div class="radio-inline">
                                    <label class="radio ">
                                        <input type="radio"  {{ $display->status == 'inactive' ? 'checked' : '' }} checked="checked" name="status" value="active" />
                                        <span></span>
                                        {{ __('lang.active') }}
                                    </label>
                                    <label class="radio">
                                        <input type="radio"  {{ $display->status == 'inactive' ? 'checked' : '' }} name="status" value="inactive" />
                                        <span></span>
                                        {{ __('lang.inactive') }}
                                    </label>
                                </div>
        ...
        <div class=" col-lg-8">
            <div class="card card-custom gutter-b">
                <div class="card-header">
                    <h3 class="card-title">{{ __('lang.display_list') }}</h3>
                </div>
                <div class="card-body p-2">

                    <form action="{{ route('master.display.filter') }}" method="post">
                        @csrf
                        @php
                            $name = $_GET['name'] ?? '';
                            $status = $_GET['status'] ?? '';
                                                    ...
                                                    <ul class="navi navi-hover">
                                                        <li class="navi-item">
                                                            @if ($data->status == 'active')
                                                                <a href="{{ route('master.display.status', $data->id) }}"
                                                                    class="navi-link ">
                                                        ...
                                                        <li class="navi-item">
                                                            <button type="button" title="{{ __('lang.view') }}"
                                                                ...
                                                                onclick="viewModalShow(`{{ route('master.display.show', $data->id) }}`)">
                                                                <span class="navi-icon">
                                                        ...
                                                        <li class="navi-item">
                                                            <a href="{{ route('master.display.edit', $data->id) }}"title="{{ __('lang.edit') }}"
                    ...
                    {{ $displays->withQueryString()->links('pagination.pagination', ['paginator' => $displays]) }}
                </div>
            </div>
        </div>
    </div>
