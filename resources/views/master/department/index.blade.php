...
@section('toolbar-right')
    <span class="text-muted font-weight-bold mr-4">{{ __('lang.department') }}</span>
@endsection
        ...
        <div class="col-lg-4">
            <div class="card card-custom gutter-b">
                <div class="card-header px-2">
                    <h3 class="card-title">{{ __('lang.add_department') }}</h3>
                </div>
                <form class="form" action="{{ route('master.department.store') }}" method="POST">
                    @csrf
        ...
        <div class=" col-lg-8">
            <div class="card card-custom gutter-b">
                <div class="card-header">
                    <h3 class="card-title">{{ __('lang.department_list') }}</h3>
                </div>
                <div class="card-body p-2">

                    <form action="{{ route('master.department.filter') }}" method="post">
                        @csrf
                        @php
                            $name = $_GET['name'] ?? '';
                            $status = $_GET['status'] ?? '';
                                                    ...
                                                    <ul class="navi navi-hover">
                                                        <li class="navi-item">
                                                            @if ($data->status == 'active')
                                                                <a href="{{ route('master.department.status', $data->id) }}" class="navi-link ">
                                                        ...
                                                        <li class="navi-item">
                                                            <button type="button" title="{{ __('lang.view') }}"
                                                                ...
                                                                onclick="viewModalShow(`{{ route('master.department.show', $data->id) }}`)">
                                                                <span class="navi-icon">
                                                        ...
                                                        <li class="navi-item">
                                                            <button type="button"
                                                                data-url="{{ route('master.department.edit', $data->id) }}"
                                                                title="{{ __('lang.edit') }}"
                    ...
                    {{ $departments->withQueryString()->links('pagination.pagination', ['paginator' => $departments]) }}
                </div>
            </div>
        </div>
    </div>
