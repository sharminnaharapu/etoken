...
@section('toolbar-right')
    <span class="text-muted font-weight-bold mr-4">{{ __('lang.edit_user') }}</span>
@endsection
        ...
        <div class=" col-lg-12">
            <div class="card card-custom gutter-b">
                <div class="card-header">
                    <h3 class="card-title">{{ __('lang.edit_user') }}</h3>
                    <a href="{{ route('master.user.index') }}"
                        class=" btn btn-outline-primary btn-sm card-title">{{ __('lang.user_list') }}</a>
                </div>
                <div class="card-body p-2">

                    <form id="userstore" action="{{ route('master.user.update', $user->id) }}" method="post"
                        ...
                        <div class="form-group row mt-0">
                            <div class="col-lg-4">
                                ...
                                <input class="form-control " required name="name" type="text"
                                    value="{{ $user->name }}" placeholder="{{ __('lang.name') }}" />
                            </div>
                            <div class="col-lg-4">
                            ...
                            <div class="col-lg-4">
                                ...
                                <select class="form-control text-capitalize" required name="doctor" id="doctor">
                                    <option value="no" {{ $user->doctor == 'no' ? 'selected' : '' }}>{{ __('lang.no') }}
                                    </option>
                                    <option value="yes" {{ $user->doctor == 'yes' ? 'selected' : '' }}> {{ __('lang.yes') }}
                                    </option>
                                </select>
                            </div>
                            <div class="col-lg-4">
                                ...
                                <select class="form-control text-capitalize currency" required name="department"
                                    ...
                                    @foreach ($departments as $data)
                                        <option value="{{ $data->id }}"
                                            {{ $data->id == $user->department_id ? 'selected' : '' }}>{{ $data->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-lg-4">
                                ...
                                <select class="form-control text-capitalize currency" required name="service"
                                    id="service">
                                    <option value="{{ $user->service_id }}">{{ $user->service->name }}</option>
                                </select>
                            </div>
                            <div class="col-lg-4" id="counterData">
                                ...
                                <select class="form-control text-capitalize currency" name="counter" id="counter">
                                    <option value="{{ $user->counter_id }}">{{ $user->counter->name }}</option>
                            ...
                            <div class="col-lg-4 d-none" id="roomData">
                                ...
                                <select class="form-control text-capitalize currency" name="room" id="room">
                                    ...
                                    @foreach ($rooms as $data)
                                        <option value="{{ $data->id }}"
                                            {{ $data->id == $user->room_id ? 'selected' : '' }}>{{ $data->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-lg-4">
                                ...
                                <select class="form-control text-capitalize " name="gender" id="gender">
                                    ...
                                    @foreach (gender() as $k => $data)
                                        <option value="{{ $k }}" {{ $k == $user->gender ? 'selected' : '' }}>
                                            {{ $data }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-lg-4">
                                ...
                                <input class="form-control datepicker" name="date_of_birth" type="date"
                                    value="{{$user->date_of_birth }}" placeholder="{{ __('lang.date_of_birth') }}" />
                            </div>
                            <div class="col-lg-4">
                            ...
                            <div class="col-lg-4">
                                ...
                                <input class="form-control " name="note" type="text" value="{{ $user->note }}"
                            ...
                            <div class="row justify-content-center col-12">
                                <div class="col-6">
                                    <button type="submit" class="btn btn-primary btn-block"
                                        id="save_data_btn">{{ __('lang.update') }}</button>
                                </div>
                            </div>
                        </div>
                    </form>
